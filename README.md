...
- **`adr/MMMM-title.md`** is one architectural or process decision per file, in Nygard format (Status, Context, Decision, Consequences). ADRs are append-only once `Accepted` - a change of mind is a *new* ADR that marks the old one `Superseded`, never an edit to a decided one. Most ADRs are feature-scoped (`docs/groundwork/features/NNNN-slug/adr/`), written by `survey` as a feature is designed. `improve-codebase-architecture` also writes ADRs for decisions that aren't tied to one feature, into a sibling `docs/groundwork/adr/`, sequenced independently.
