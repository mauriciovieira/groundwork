...
**Triage labels.** Optional. Only meaningful for `github`/`linear`. A short list of labels `to-issues` applies to every issue it creates (e.g. `groundwork`) - pick something that doesn't collide with any of `triage`'s five canonical state-role labels below, or its issues will be indistinguishable from ones still awaiting triage. Skip for `local`.
