...
An open issue is workable whether it came from `to-issues` (a `Type` field somewhere in the issue body, pointing back to a `prd.md`) or from `/groundwork:triage` (a `**Type:** AFK`/`HITL` line inside its own Agent Brief comment, with no PRD at all) - the exact formatting isn't guaranteed to match between the two, so look for a `Type` field wherever it appears rather than a specific markdown shape, and treat the two sources identically once found.
