---
name: brainstorm
description: Use when the user brings a fuzzy or new idea - "let's build X", "I want a feature that...", "what if we..." - before any design or implementation starts. Socratic discovery, one question at a time; never proposes a solution before the problem is crisp. Hands off to inception or survey when ready.
---
...
If partway through it becomes clear this is actually small - a one-file fix, a copy change, something with no real design space - say so and, if the user agrees, switch into the `quick` skill directly instead of continuing the interview. Don't force a small thing through a big process, and don't make the user type the command.
...
Ask which fits rather than assuming, then continue straight into the chosen skill in this same session - invoke it, don't wait for the user to type the slash command. Do not start writing PRDs, ADRs, or code inside *this* skill - that's the next orchestrator's job.
