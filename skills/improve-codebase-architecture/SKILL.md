...
The report uses **Tailwind via CDN** for layout and styling, and **Mermaid via CDN** for diagrams where a graph/flow/sequence reliably communicates the structure. Mix Mermaid with hand-crafted CSS/SVG visuals - use Mermaid when relationships are graph-shaped (call graphs, dependencies, sequences), and hand-built divs/SVG when you want something more editorial (mass diagrams, cross-sections, before/after call-graph collapse). Each candidate gets a **before/after visualization** - static side-by-side, not an animation; the report has no interactivity beyond Mermaid's own rendering. Be visual.
...
- **Want to explore alternative interfaces for the deepened module?** See [INTERFACE-DESIGN.md](INTERFACE-DESIGN.md), which specialises the competing-designs pattern in `../_shared/COMPETING-DESIGNS.md` - several candidates produced independently, then ranked by a worker that did not write any of them.
